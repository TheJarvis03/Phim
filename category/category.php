            ...
            // Xử lý hiển thị phim
                if (isset($_GET['xem'])) {
                    ...
                    if(isset($_GET['search']) && !empty($_GET['search'])){
                        ...
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Tenphim LIKE '%$searchInput%' OR Movie LIKE '%$searchInput%'", 12, $currentPage);
                ...
                switch ($temp) {
                    ...
                    case 'hai':
                        ...
                        break;
                    case 'kinhdi':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Kinh dị%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break;
                    case 'langman':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Lãng mạn%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break;
                    case 'giadinh':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Gia Đình%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break;
                    case 'chientranh':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Chiến tranh%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break;
                    case 'cotrang':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Vũ trụ%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break;
                    case 'phieuluu':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Phiêu lưu%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break; 
                    case 'hinhsu':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Hình sự%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break;  
                    case 'khoahoc':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Khoa học%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break; 
                    case 'chinhkich':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Chính kịch%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break;  
                    case 'tamly':
                        showMoviesWithPagination("SELECT Tenphim, Movie, poster FROM Content WHERE Theloai LIKE '%Tâm lý%' ORDER BY Tenphim ASC", 12, $currentPage);
                        break;      
                    case 'tam':
